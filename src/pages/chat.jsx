...
const Chat = ({ isDark = false, setIsDark }) => {
  ...
  if (!full) {
    pushBotError(
      "Explain failed",
      "Pehle koi math problem upload ya enter karo."
    );
    return;
  }

  const style = resolveReplyStyleForMessage(full);

  // Extract only the actual math problem from OCR text
  const problem = getProblemForSolve(full) || full;

  pushUserMessage(full, imageUrl);
  ...
  try {
    // Directly call the explanation API.
    // Do NOT use fetchBotReply here because that may route
    // the request through normal chat instead of explain.
    const cachedAnswer =
      lastSolvedRef.current?.question === problem
        ? lastSolvedRef.current.answer
        : null;

    const data = await explainSteps(
      problem,
      style,
      cachedAnswer
    );
    ...
    pushBotMessage(formatExplanation(data, style));
  } catch (err) {
    pushBotError(
      "Explain failed",
      err.message === "Failed to fetch"
